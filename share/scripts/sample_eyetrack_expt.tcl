...
######################################################################
# Sample Tcl code for psychophysics experiments with the ISCAN
# eyetracker. This script sets up and runs an experiment consisting of
# a sequence of free-viewing trials. Each trial consists of, in
# sequence, (1) a fixation cross, (2) an image, and (3) a response
# screen in which the user is asked to press either 1 or 2 indicating
...
######################################################################
# Make a grid of points for calibration sequences

proc make_grid { x_min x_max x_npts y_min y_max y_npts } {
    set x_points [dlist::linspace $x_min $x_max $x_npts]
    set y_points [dlist::linspace $y_min $y_max $y_npts]

    set result [list]

    foreach x_pt $x_points {
        foreach y_pt $y_points {
            lappend result [list $x_pt $y_pt 0]
        }
    }
    return $result
}
...
######################################################################
# Make a block consisting of a series of calibration points

proc make_calibration_block { toglet num_points serport } {

    # These are necessary for later processing of the log file
    Log::log "width [-> $toglet width]"
    Log::log "height [-> $toglet height]"
    ...
    switch $num_points {
        25 {
            ...
            set bottom -400
            set top 400
            set coords [make_grid $left $right 5 $bottom $top 5]
            set coords [dlist::shuffle $coords [clock clicks]]
        }
        default {
            error "unsupported number of points $num_points"
        }
    }

    Log::log "num points $num_points"

    # The newline separator is key for proper awk parsing
    Log::log [join $coords "\n"]

    set small_fixpt [new GxSeparator \
                         [list addChildren \
                              [list [new GxColor {rgba {0 1 1 1}}] \
                                   [new FixPt {length 8 width 5}]]]]

    set fixpt [new GxSeparator \
                   [list addChildren \
                        [list [new GxColor {rgba {1 1 0 1}}] \
                             [new FixPt {length 18 width 2}]]]]
    ...
    foreach coord $coords {
        set pos [new GxTransform [list translation $coord]]

        set trial [new Trial]

        -> $trial info "fixpt \#${i} at ([join $coord ,])"

        -> $trial addNode [new GxSeparator \
                               [list addChildren [list $pos $small_fixpt]]]
        -> $trial addNode [new GxSeparator \
                               [list addChildren [list $pos $fixpt]]]
    ...
    -> $th addStartEvent [new RenderBackEvent "delay 5"]

    -> $th addStartEvent [new RenderEvent "delay 15"]
    -> $th addStartEvent [new SwapBuffersEvent "delay 50"]
    -> $th addStartEvent [new ClearBufferEvent "delay 60"]
    -> $th addStartEvent [new SwapBuffersEvent "delay 150"]

    -> $th addStartEvent [new RenderEvent "delay 165"]
    -> $th addStartEvent [new SwapBuffersEvent "delay 250"]
    -> $th addStartEvent [new ClearBufferEvent "delay 260"]
    -> $th addStartEvent [new SwapBuffersEvent "delay 350"]

    -> $th addStartEvent [new RenderEvent "delay 365"]
    -> $th addStartEvent [new SwapBuffersEvent "delay 450"]
    -> $th addStartEvent [new ClearBufferEvent "delay 460"]
    -> $th addStartEvent [new SwapBuffersEvent "delay 550"]

    # Here we display the final fixation cross
    -> $th addStartEvent [new NextNodeEvent "delay 560"]
    -> $th addStartEvent [new ClearBufferEvent "delay 570"]
    -> $th addStartEvent [new RenderEvent "delay 600"]
    set sb [new MultiEvent "delay 650"]
    -> $sb addEvents [list \
                          [new SwapBuffersEvent] \
                          [new FinishDrawingEvent] \
                          [new FileWriteEvent "byte 95 file $serport"]]
    -> $th addStartEvent $sb

    # Here we wait for the user's response
    -> $th addStartEvent [new AllowResponsesEvent "delay 3000"]

    # On the user's response, we clear the fixation cross, write a final
    # byte to the serport and then move on to the next trial

    set sb [new MultiEvent "delay 1"]
    -> $sb addEvents [list \
                          [new FileWriteEvent "byte 96 file $serport"] \
    ...
    -> $trials responseHdlr [make_spacebar_responsehdlr]
...
"CALIBRATION SEQUENCE
 ...
 (1) you will see a series of fixation crosses
 (2) carefully fixate each fixation cross as it appears
 (3) if you need to blink, do so at the beginning of when each cross appears
 (4) once you have steadily fixated the cross for two seconds WITHOUT BLINKING,
     press the spacebar to go on to the next cross

\[press the spacebar to continue\]"

    -> $block addElements [make_text_trial $instructions {0.8 0.8 0.8 1.0}]
...
proc make_main_timinghdlr { serport image_duration } {
    ...
    -> $sb addEvents [list \
                          [new SwapBuffersEvent] \
                          [new FinishDrawingEvent] \
                          [new FileWriteEvent "byte 97 file $serport"]]
    -> $th addStartEvent $sb
    ...
    -> $sb addEvents [list \
                          [new SwapBuffersEvent] \
                          [new FinishDrawingEvent] \
                          [new FileWriteEvent "byte 98 file $serport"]]
    -> $th addStartEvent $sb
    ...
    -> $sb addEvents [list \
                          ...
                          [new FileWriteEvent "byte 99 file $serport"]]
    -> $th addStartEvent $sb
    ...
    -> $sb addEvents [list \
                          ...
                          [new SwapBuffersEvent] \
                          [new FinishDrawingEvent] \
                          [new FileWriteEvent "byte 100 file $serport"]]
    -> $th addResponseEvent $sb
...
proc make_main_block { image_files serport {image_duration 3000} } {
    ...
    set th [make_main_timinghdlr $serport $image_duration]
...
######################################################################
# make a block object that starts the eyetracker recording

proc make_start_block { serport } {

    set th [new TimingHdlr]
    set ev [new MultiEvent "delay 1"]
    -> $ev addEvents [list \
                          [new FileWriteEvent "byte 132 file $serport"] \
                          [new EndTrialEvent]]
    -> $th addStartEvent $ev

    set trial [new Trial]
    -> $trial info "start eyetracker recording"
    -> $trial timingHdlr $th
    -> $trial responseHdlr [new NullResponseHdlr]

    set block [new Block]
    -> $block addElement $trial
...
######################################################################
# make a block object that stops the eyetracker recording

proc make_stop_block { serport } {

    set th [new TimingHdlr]
    set ev [new MultiEvent "delay 1"]
    -> $ev addEvents [list \
                          [new FileWriteEvent "byte 136 file $serport"] \
                          [new EndTrialEvent]]
    -> $th addStartEvent $ev

    set trial [new Trial]
    -> $trial info "stop eyetracker recording"
    -> $trial timingHdlr $th
    -> $trial responseHdlr [new NullResponseHdlr]
...
proc main { argv } {
    ...
    set serport [new OutputFile {filename "/dev/cua0"}]
    ...
    if { $RECORD } {
        -> $expt addElement [make_start_block $serport]
    }

    if { $CALIB } {
        -> $expt addElement [make_calibration_block $toglet 25 $serport]
    }

    -> $expt addElement [make_main_block $image_files $serport]
    ...
    if { $RECORD } {
        -> $expt addElement [make_stop_block $serport]
    }
